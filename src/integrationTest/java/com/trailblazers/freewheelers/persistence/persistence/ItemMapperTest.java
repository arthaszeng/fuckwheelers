...
public class ItemMapperTest extends MapperTestBase {
    ...
    @Test
    public void shouldDecreaseNumbersSuccessfully() {
        Item originItem = someItem();
        itemMapper.insert(originItem);
        Long originQuantity = originItem.getQuantity();
        int three = 3;

        itemMapper.decreaseQuantity(originItem.getItemId(), three);

        assertThat(itemMapper.get(originItem.getItemId()).getQuantity(), is(originQuantity - three));
    }
